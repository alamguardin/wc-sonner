...
export default class Toast {
	...
	setToaster(arr) {
		if (arr.length > 0) {
			...
			throw new Error('No wc-sonner instances have been created');
		}
	}
	...
	#renderToast(data) {
		...
		if (data.sonnerId) {
			const seletedToaster = Array.from(this._toaster).find((element) => {
				return element.id === data.sonnerId;
			});
			...
			if (seletedToaster) {
				...
				throw new Error(
					`Not finding wc-sonner with ${data.sonnerId} id`,
				);
			}
		}
	}
	...
	display(message, options = {}) {
		const toastData = {
			...
			sonnerId: options.sonnerId ?? undefined,
		};
